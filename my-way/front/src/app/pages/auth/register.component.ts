    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 flex items-center justify-center p-4">
      <div class="w-full max-w-md anim-fade-up">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 sm:p-8">
          <h1 class="text-xl font-bold text-slate-900 mb-1">Créer mon espace gratuit</h1>
          <p class="text-sm text-slate-500 mb-6">2 minutes, sans engagement, 100% gratuit.</p>

          <form (ngSubmit)="submit()" class="space-y-4">
            <div>
              <label class="ui-label text-sm font-semibold text-slate-700">Nom complet</label>
              <input
                type="text" name="fullName" required autocomplete="name"
                [(ngModel)]="fullName" class="ui-input mt-1.5" placeholder="Prénom Nom" />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                [(ngModel)]="password" class="ui-input mt-1.5" placeholder="8 caractères minimum" />
            </div>
            ...
            <button
              ...
              class="ui-btn w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white py-5 rounded-xl shadow-lg shadow-emerald-500/30">
              @if (loading) {
                ...
                <span class="flex items-center gap-2">
                  <lucide-icon [img]="ic.UserPlus" class="w-4 h-4" />
                  Créer mon compte
                </span>
              }
            </button>
          ...
          <!-- Inscription Google (bientôt active) -->
          <button
            type="button"
            (click)="googleSoon()"
...
export class RegisterComponent implements OnInit {
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      this.router.navigateByUrl(from || '/Onboarding');
    } catch (err: unknown) {
